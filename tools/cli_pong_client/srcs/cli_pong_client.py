...
class PaddleControl:
        ...
        while True:
            ...
            if self.game_id.isnumeric():
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type a number. "
                )
        Utils.print_colored_message(
            "yellow", f'\nOK. The Game id is \n\n" ----- ' + self.game_id + ' ----- "\n'
        )
    ...
    def post_game_setting(self, ball_velocity, ball_size, map):
        ...
        try:
            response = requests.post(
                game_setting_url, json=game_setting, headers=headers, verify=False
            )
            ...
            if response.status_code == 201:
                ...
                Utils.print_colored_message(
                    "yellow",
                    f'Game created successfully. The Game id is \n\n" ----- '
                    + str(self.game_id)
                    + ' ----- "\n',
                )
            else:
                Utils.print_colored_message(
                    "red",
                    f"Failed to create game. Server responded with status code {response.status_code}.",
                )
                sys.exit(1)
            ...
            # リクエストのエラー（接続エラーなど）
            Utils.print_colored_message(
                "red", f"Error occurred while creating game: {str(e)}"
            )
            sys.exit(1)
    ...
    def create_game(self):
        ...
        while True:
            ...
            if user_input == "1":
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type 1(Fast) or 2(Normal) or 3(Slow)."
                )

        Utils.print_colored_message(
            "green", "Select ball size.\nType 1(Big), 2(Normal), or 3(Small)"
        )
        Utils.print_colored_message("red", "(Press Q to quit.)")
        while True:
            user_input = sys.stdin.readline().strip()
            if user_input == "1":
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type 1(Big) or 2(Normal) or 3(Small)."
                )
        ...
        Utils.print_colored_message(
            "white", "Please access the following URL from the browser."
        )
        Utils.print_colored_message(
            "yellow", f"https://localhost:8443/#/gameplay.{self.game_id}/\n"
        )

    def first_setup(self):
        Utils.print_colored_message("green", "Welcome to Pong Game!!!\n")
        Utils.print_colored_message(
            "green", "Press 1 to join an existing game, or 2 to create your own!"
        )
        Utils.print_colored_message("red", "(Press Q to quit.)")
        while True:
            user_input = sys.stdin.readline().strip()
            if user_input == "1":
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type 1(Join) or 2(Create)."
                )

        Utils.print_colored_message(
            "green", "Which paddle do you want to control?\nType D(Left) or K(Right)"
        )
        Utils.print_colored_message("red", "(Press Q to quit.)")
        while True:
            user_input = sys.stdin.readline().strip()
            ...
            if user_input in ["D", "d"]:
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type D(Left) or K(Right)."
                )
        Utils.print_colored_message(
            "yellow",
            '\nOK. You control \n\n" ----- '
    ...
    def post_login(self):
        ...
        try:
            response = requests.post(
                login_url, headers=headers, json=login_info, verify=False
            )
    ...
    def login(self):
        ...
        while True:
            ...
            if user_input in ["Y", "y"]:
                ...
                Utils.print_colored_message(
                    "red", "Invalid input. Please type Y(Yes) or N(No)."
                )
