...
class TournamentContract:
    ...
    def get_match(self, tournament_id, match_number):
        match_details = self.contract.functions.getMatch(
...
def record_match_on_blockchain(
    ...
    try:
        ...
        # トーナメントの存在確認
        try:
            tournament_details = tournament_contract.contract.functions.tournaments(
                tournament_id
            ).call()
            if not tournament_details[0]:  # トーナメントIDが0の場合は存在しないとみなす
                logger.info(
                    f"Tournament with ID {tournament_id} does not exist. Creating new tournament."
                )
                tournament_contract.create_tournament(
                    f"Auto Tournament {tournament_id}", timestamp
                )
        except Exception as e:
