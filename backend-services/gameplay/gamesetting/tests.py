...
class GameSettingViewTest(TestCase):
    def setUp(self):
        ...
        self.game_setting = GameSetting.objects.create(
            ball_velocity="normal", ball_size="normal", map="a"
        )
    ...
    def test_get(self):
        response = self.client.get(
            reverse("gamesetting:game_settings_list"), follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(GameSetting.objects.last().ball_velocity, "normal")

    def test_post(self):
        data = {"ball_velocity": "fast", "ball_size": "big", "map": "b"}
        response = self.client.post(
            reverse("gamesetting:game_settings_list"), data, format="json"
        )
    ...
    def test_put(self):
        data = {"ball_velocity": "slow", "ball_size": "small", "map": "c"}
        response = self.client.put(
            reverse(
                "gamesetting:game_settings_detail", kwargs={"pk": self.game_setting.id}
            ),
            data,
            format="json",
        )
