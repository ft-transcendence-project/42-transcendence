...
class PongLogic(AsyncWebsocketConsumer):
        ...
        async with self.pong_info.lock:
            if (
                ...
                if self.pong_info.score.left >= SCORE_TO_WIN:
                    ...
                    await self.send_game_over_message("game_over")
    ...
    async def connect(self):
        ...
        if setting_id in self.pong_info_map:
            ...
            async with self.pong_info.lock:
                if self.pong_info.is_remote == True and self.pong_info.is_end == True:
                    if self.pong_info.winner == "left":
                        await self.send_channel_message(
                            self.channel_name, {"type": "reload", "winner": "left"}
                        )
                    elif self.pong_info.winner == "right":
                        await self.send_channel_message(
                            self.channel_name, {"type": "reload", "winner": "right"}
                        )
        ...
        print(
            f"{self.pong_info.setting_id} -> connect player_cnt: {self.pong_info.player_cnt}"
        )
    ...
    async def disconnect(self, close_code):
        ...
        async with self.pong_info.lock:
            ...
            if self.pong_info.player_cnt > 0:
                self.pong_info.player_cnt -= 1
                print(
                    f"{setting_id} -> disconnect player_cnt: {self.pong_info.player_cnt}"
                )
            if self.pong_info.is_remote == False or self.pong_info.player_cnt == 0:
                self.pong_info.task["game_loop"].cancel()
                if setting_id in self.pong_info_map:
                    del self.pong_info_map[setting_id]
                return
            if self.pong_info.is_remote == True and self.pong_info.is_end == False:
                ...
                if self.channel_name == self.pong_info.remote_left["channel_name"]:
                    remaining_player = self.pong_info.remote_right["channel_name"]
                    self.pong_info.remote_left["status"] = False
                    self.pong_info.winner = "right"
                else:
                    remaining_player = self.pong_info.remote_left["channel_name"]
                    self.pong_info.remote_right["status"] = False
                    self.pong_info.winner = "left"
                if self.pong_info.is_game_started == True:
                    self.pong_info.is_end = True
                    await self.send_game_over_message("interrupted")
                else:
                    await self.send_channel_message(
                        remaining_player, {"type": "interrupted before start"}
                    )
    ...
    async def set_remote_mode(self, pong_data):
        if pong_data.get("remote_player_pos", None) == "right":
            self.pong_info.remote_right = {
                "status": True,
                "channel_name": self.channel_name,
            }
        elif pong_data.get("remote_player_pos", None) == "left":
            self.pong_info.remote_left = {
                "status": True,
                "channel_name": self.channel_name,
            }
        if (
            self.pong_info.player_cnt == 2
            and self.pong_info.remote_left["channel_name"]
            != self.pong_info.remote_right["channel_name"]
            and self.pong_info.remote_left["status"] == True
            and self.pong_info.remote_right["status"] == True
        ):
            await self.send_channel_message(
                self.pong_info.remote_left["channel_name"],
                {"type": "remote_OK", "player": "left"},
            )
            await self.send_channel_message(
                self.pong_info.remote_right["channel_name"],
                {"type": "remote_OK", "player": "right"},
            )
    ...
    async def send_game_over_message(self, message):
        await self.channel_layer.group_send(
            self.pong_info.group_name,
            {
                "type": "send_message",
                "content": Utils.generate_game_over_message(self.pong_info, message),
            },
        )

    # self.sendは、送信するデータの種類を指定するために、text_data or bytes_dataを必要とする。
