...
class PongLogic(AsyncWebsocketConsumer):
        ...
        async with self.pong_info.lock:
            if (
                ...
                if self.pong_info.score.left >= SCORE_TO_WIN:
                    ...
                    await self.send_game_over_message("left","game_over")
                ...
                if self.pong_info.score.right >= SCORE_TO_WIN:
                    self.pong_info.is_end = True
                    await self.send_game_over_message("right","game_over")
        ...
        if setting_id in self.pong_info_map:
            ...
            async with self.pong_info.lock:
                if self.pong_info.is_remote == True and self.pong_info.is_end == True:
                    await self.send_channel_message(self.channel_name, {"type":"reload"})
        ...
        async with self.pong_info.lock:
            ...
            if self.pong_info.is_remote == True and self.pong_info.is_end == False:
                ...
                if self.pong_info.is_game_started == True:
                    self.pong_info.is_end = True
                    await self.send_game_over_message(winner,"interrupted")
                else:
                    await self.send_channel_message(remaining_player, {
    ...
    async def send_game_over_message(self, winner,message):
        await self.channel_layer.group_send(
            ...
            {
                "type": "send_message",
                "content": Utils.generate_game_over_message(self.pong_info,winner,message),
            },
        )
