...
contract("TournamentData", (accounts) => {
  ...
  it("should create a player", async () => {
    ...
    assert.equal(
      player.name,
      playerName,
      "Player name should be recorded correctly",
    );
  });
  ...
  it("should create a tournament", async () => {
    ...
    const date = Date.now();
    await tournamentInstance.createTournament(tournamentName, date, {
      from: owner,
    });
    ...
    assert.equal(
      tournamentData.name,
      tournamentName,
      "Tournament name should be recorded correctly",
    );
    assert.equal(
      tournamentData.date.toNumber(),
      date,
      "Tournament date should be recorded correctly",
    );
  });
  ...
  it("should record a match by owner", async () => {
    ...
    await tournamentInstance.createTournament("Tournament1", timestamp, {
      from: owner,
    });
    ...
    assert.equal(
      match.player1Id,
      player1Id,
      "Player 1 ID should be recorded correctly",
    );
    assert.equal(
      match.player2Id,
      player2Id,
      "Player 2 ID should be recorded correctly",
    );
    assert.equal(
      match.player1Score,
      player1Score,
      "Player 1 score should be recorded correctly",
    );
    assert.equal(
      match.player2Score,
      player2Score,
      "Player 2 score should be recorded correctly",
    );
  });

  it("should not allow non-owner to record a match", async () => {
    try {
      await tournamentInstance.recordMatch(1, 1, 1, Date.now(), 1, 2, 10, 8, {
        from: nonOwner,
      });
      ...
      assert(
        ...
        "Expected 'Caller is not the owner' error",
      );
    }
  });
  ...
  it("should emit MatchRecorded event when a match is recorded", async () => {
    ...
    assert.equal(
      result.logs[0].event,
      "MatchRecorded",
      "Event should be 'MatchRecorded'",
    );
    assert.equal(
      result.logs[0].args.round,
      round,
      "Round should be correct in event",
    );
    assert.equal(
      result.logs[0].args.matchNumber,
      matchNumber,
      "Match number should be correct in event",
    );
    assert.equal(
      result.logs[0].args.player1Id,
      player1Id,
      "Player 1 ID should be correct in event",
    );
    assert.equal(
      result.logs[0].args.player2Id,
      player2Id,
      "Player 2 ID should be correct in event",
    );
    assert.equal(
      result.logs[0].args.player1Score,
      player1Score,
      "Player 1 score should be correct in event",
    );
    assert.equal(
      result.logs[0].args.player2Score,
      player2Score,
      "Player 2 score should be correct in event",
    );
  });
  ...
  it("should increment matchCount when a match is recorded", async () => {
    ...
    await tournamentInstance.createPlayer("Player1", { from: owner });
    await tournamentInstance.createPlayer("Player2", { from: owner });
    await tournamentInstance.createTournament("Tournament1", Date.now(), {
      from: owner,
    });
    ...
    assert.equal(
      newMatchCount.toNumber(),
      1,
      "Match count should be incremented to 1",
    );

    await tournamentInstance.recordMatch(1, 1, 2, Date.now(), 1, 2, 10, 8, {
      from: owner,
    });
    const finalMatchCount = await tournamentInstance.matchCount();
    assert.equal(
      finalMatchCount.toNumber(),
      2,
      "Match count should be incremented to 2",
    );
  });
});
